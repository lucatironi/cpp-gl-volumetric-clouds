...
class Texture3D
{
    public:
        ...
        Texture3D(GLuint width, GLuint height, GLuint depth, GLuint wrap, GLuint filterMin, GLuint filterMax) : Width(width), Height(height), Depth(depth),
            InternalFormat(GL_RGBA32F), ImageFormat(GL_RGBA),
            WrapS(wrap), WrapT(wrap),
            FilterMin(filterMin), FilterMax(filterMax)
        {
            // Now generate texture
        ...
        void Generate()
        {
            ...
            glTexImage3D(GL_TEXTURE_3D, 0, InternalFormat, Width, Height, Depth, 0, ImageFormat, GL_FLOAT, NULL);
